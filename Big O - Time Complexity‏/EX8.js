const findIndex = function(numbers, num) {
  ...
  while (left <= right) {
    //Find the middle index of our current search 
    let mid = Math.floor((left + right) / 2);
    if (numbers[mid] === num) {
      return mid; // We found the number
    } 
    else if (numbers[mid] < num) {
      //  middle number is too small, so our target MUST be to the right
      left = mid + 1; 
    } 
    else {
      //  middle number is too big, so our target MUST be to the left.
      right = mid - 1; 
    }
  }
  return -1; 
